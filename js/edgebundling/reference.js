const EdgeBundling = function (app) {
    ...
    const nodesNumber = this.app.treeData.children
        .map((d) => d.children.length)
        .reduce((sum, x) => sum + x);

    const radius =
        ((nodesNumber + this.app.treeData.children.length) * (props.nodeFontSize + props.nodeMargin)) / (2 * Math.PI); // props.windowHeight / 2;
    ...
    const root = tree(bilink(d3.hierarchy(this.app.treeData)));
